...
An attempt to try to improve a little on the orginal design

This library implements SNMP (the Simple Network Management Protocol).  It is
implemented in pure Ruby, so there are no dependencies <b>(read only one on libsmi to import MIBs)</b> on external libraries
